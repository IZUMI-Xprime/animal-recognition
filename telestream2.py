...
# ESP32 stream URL
esp32_stream_url = "http://192.168.0.108:81/stream" # Replace with your ESP32 stream URL
...
def generate_heatmap():
    data = np.random.rand(10, 10)  # Random 10x10 heatmap data
    plt.figure(figsize=(6, 5))
    sns.heatmap(data, cmap='coolwarm', annot=True)

    # Ensure the 'static' folder exists to save images
...
@app.route('/')
def index():
    heatmap_path = generate_heatmap()  # Generate heatmap dynamically
    return render_template('index.html', heatmap_url=heatmap_path)
